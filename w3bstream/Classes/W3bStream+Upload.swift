...
public extension W3bStream {
    ...
    func makeNewAPIPayload(payload: String, publisherKey: String, publishToken: String) -> [String: Any] {
        let event_type = "2147483647"
        let dic = [
            "header": ["event_type": event_type,
                       "pub_id": publisherKey,
                       "pub_time": Date().timeIntervalSince1970,
                       "token": publishToken],
            "payload": payload
        ] as [String : Any]
        return dic
    ...
    func upload(url: URL?=nil, payload: String, publisherKey: String, publisherToken: String, completionHandler: ((Data?, Error?) -> Void)?) {
        let targetURLs = url != nil ? [url!] : Config.shared.httpsUrls
        targetURLs.forEach { url in
            let newPayload = makeNewAPIPayload(payload: payload, publisherKey: publisherKey, publishToken: publisherToken)
